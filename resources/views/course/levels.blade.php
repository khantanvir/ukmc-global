        ...
        <div class="widget-content widget-content-area">
            <form method="post" action="{{ URL::to('course-level-store') }}" enctype="multipart/form-data">
                @csrf
                 <div class="row mb-4">
                     <div class="col-7">
                         <input type="hidden" name="level_id" value="{{ (!empty($level->id))?$level->id:'' }}" />
                         <input type="text" name="title" value="{{ (!empty($level->title))?$level->title:'' }}" class="form-control" placeholder="Course Level Name">
                     </div>
                     <div class="col">
                ...
                <div class="widget-content widget-content-area br-8">
                    <table id="zero-config" class="table dt-table-hover text-center" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr class="{{ (!empty($return_level_id) && $return_level_id==$row->id)?'tr-bg':'' }}">
                                ...
                                <td>
                                    <div class="switch form-switch-custom switch-inline form-switch-primary form-switch-custom inner-text-toggle">
                                        <div class="input-checkbox">
                                            ...
                                            <input {{ ($row->status==0)?'checked':'' }} data-action="{{ URL::to('level-status-change') }}" data-id="{{ $row->id }}" class="level-status-change switch-input" type="checkbox"
                                ...
                                <td>
                                    <a href="{{ URL::to('course-levels/'.$row->id) }}" class="badge badge-pill bg-warning">
