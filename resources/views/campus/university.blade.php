...
<div class="layout-px-spacing">
    <div class="middle-content container-xxl p-0">
        ...
        <h5 class="p-3">Universities</h5>
        <div class="widget-content widget-content-area">
            <form method="post" action="{{ URL::to('university-store') }}" enctype="multipart/form-data">
                @csrf
                 <div class="row mb-4">
                     <div class="col-7">
                         <input type="hidden" name="university_id" value="{{ (!empty($university_data->id))?$university_data->id:'' }}" />
                         <input type="text" name="title" value="{{ (!empty($university_data->title))?$university_data->title:'' }}" class="form-control" placeholder="University Name">
                     </div>
                     <div class="col">
            ...
            <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                <div class="widget-content widget-content-area br-8">
                    <table id="zero-config" class="table dt-table-hover text-center" style="width:100%">
                        ...
                        <tbody>
                            @forelse ($university_list as $row)
                            <tr class="{{ (!empty($return_university_id) && $return_university_id==$row->id)?'tr-bg':'' }}">
                                ...
                                <td>
                                    <div class="switch form-switch-custom switch-inline form-switch-primary form-switch-custom inner-text-toggle">
                                        <div class="input-checkbox">
                                            ...
                                            <input {{ ($row->status==0)?'checked':'' }} data-action="{{ URL::to('university-status-change') }}" data-id="{{ $row->id }}" class="university-status-change switch-input" type="checkbox"
                                ...
                                <td>
                                    <a href="{{ URL::to('universities/'.$row->id) }}" class="badge badge-pill bg-warning">
